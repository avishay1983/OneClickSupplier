...
def send_email_via_smtp(to_email: str, subject: str, html_content: str):
    ...
    try:
        print(f"Trying SMTP_SSL to {smtp_host}:465...")
    ...
    try:
        print(f"Trying SMTP STARTTLS to {smtp_host}:587...")
        context = ssl.create_default_context()
        with smtplib.SMTP(smtp_host, 587, timeout=30) as server:
            ...
            server.login(gmail_user, gmail_password)
            server.send_message(msg)
        print(f"Email sent successfully to {to_email} via STARTTLS:587")
        return True
    except Exception as e2:
        print(f"STARTTLS:587 also failed: {e2}")
