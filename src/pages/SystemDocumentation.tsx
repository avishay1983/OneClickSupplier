...
### תהליך קליטת ספק
1. עובד יוצר בקשה חדשה (קובע רמת אישור: מנהל רכש בלבד / מנהל רכש + סמנכ"ל)
2. לינק נשלח לספק במייל
3. ספק מאמת OTP
4. ספק מעלה מסמכים (OCR אוטומטי)
5. ספק ממלא טופס ושולח
6. מטפל מאשר/דוחה/מחזיר
7. מנהל רכש מקבל מייל + חותם (חובה תמיד)
8. סמנכ"ל מקבל מייל + חותם (רק אם נבחר בשלב 1)
9. ספק מועבר ל-CRM

### רמות אישור
- **אישור מנהל רכש בלבד:** חתימה אחת - מנהל רכש בלבד
- **אישור מנהל רכש + סמנכ"ל:** שתי חתימות - סמנכ"ל ראשון, אח"כ מנהל רכש
...
<ol>
<li>עובד יוצר בקשה חדשה (קובע רמת אישור: מנהל רכש בלבד / מנהל רכש + סמנכ"ל)</li>
<li>לינק נשלח לספק במייל</li>
<li>ספק מאמת OTP</li>
<li>ספק מעלה מסמכים (OCR אוטומטי)</li>
<li>ספק ממלא טופס ושולח</li>
<li>מטפל מאשר/דוחה/מחזיר</li>
<li>מנהל רכש מקבל מייל + חותם (חובה תמיד)</li>
<li>סמנכ"ל מקבל מייל + חותם (רק אם נבחר בשלב 1)</li>
<li>ספק מועבר ל-CRM</li>
</ol>
<h3>רמות אישור</h3>
<ul>
<li><strong>אישור מנהל רכש בלבד:</strong> חתימה אחת - מנהל רכש בלבד</li>
<li><strong>אישור מנהל רכש + סמנכ"ל:</strong> שתי חתימות - סמנכ"ל ראשון, אח"כ מנהל רכש</li>
</ul>
</div>
